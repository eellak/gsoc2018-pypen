...
for user in in_data:
    ...
    for field in in_data[user]:
        ...
        # wordlist.extend(in_data[user][field])
    ...
    # TODO form combinations in groups (words above must be in groups)

    # step3: combine fields given the rules we've defined
    to_append = []
    if fields_map['fam_rel'] in in_data[user]:
        to_append.extend(combinations_with_replacement(in_data[user][fields_map['fam_rel']] + in_data[user][fields_map['fam_rel']], 2))
        to_append.extend(combinations_with_replacement(in_data[user][fields_map['fam_rel']] + in_data[user][fields_map['fam_rel']], 3))
        to_append.extend(combinations_with_replacement(in_data[user][fields_map['fam_rel']] + in_data[user][fields_map['name']], 2))
        to_append.extend(combinations_with_replacement(in_data[user][fields_map['fam_rel']] + in_data[user][fields_map['name']], 3))
    if fields_map['other'] in in_data[user]:
        to_append.extend(combinations_with_replacement(in_data[user][fields_map['other']] + in_data[user][fields_map['name']], 2))
        to_append.extend(combinations_with_replacement(in_data[user][fields_map['other']] + in_data[user][fields_map['name']], 3))
        if fields_map['bday'] in in_data[user]:
            to_append.extend(combinations_with_replacement(in_data[user][fields_map['other']] + in_data[user][fields_map['bday']], 2))
            to_append.extend(combinations_with_replacement(in_data[user][fields_map['other']] + in_data[user][fields_map['bday']], 3))
            to_append.extend(combinations_with_replacement(in_data[user][fields_map['name']] + in_data[user][fields_map['bday']], 2))
            to_append.extend(combinations_with_replacement(in_data[user][fields_map['name']] + in_data[user][fields_map['bday']], 3))

    for item in to_append:
        wordlist.append(''.join(item))

    # TODO break tuples into phrases
    out_file = open(user+'_wordlist.txt', 'w')
